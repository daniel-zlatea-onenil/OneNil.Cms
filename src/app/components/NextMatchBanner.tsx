...
import {MatchViewModel} from "@/lib/viewModels"; // if not modular yet, inline logic
            ...
            <div className="relative z-10 text-center max-w-screen-xl mx-auto px-4">
                ...
                <div className="flex items-center justify-center space-x-4 text-lg mb-2">
                    <div className="flex items-center space-x-2">
                        <img src={viewModel.teamHome.logoUrl} alt={viewModel.teamHome.name} className="h-18 w-18 object-contain" />
                        <span>{viewModel.teamHome.name}</span>
                    </div>
                    <span>vs</span>
                    <div className="flex items-center space-x-2">
                        <img src={viewModel.teamAway.logoUrl} alt={viewModel.teamAway.name} className="h-18 w-18 object-contain" />
                        <span>{viewModel.teamAway.name}</span>
                    </div>
                </div>
                <p className="text-sm mb-2">
                    {viewModel.location} · {viewModel.kickoffTime} · {viewModel.competition}
                </p>
                <p className="text-sm mb-4">
                    {viewModel.date}
                </p>
                {viewModel.targetDate && <Countdown targetDate={viewModel.targetDate} />}
                {viewModel.slug && (
                    <a
